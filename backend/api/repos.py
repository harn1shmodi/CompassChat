...
from services.production_summarizer import production_summarizer
...
# Concurrency control: Limit concurrent repository analyses to prevent resource exhaustion
repo_analysis_semaphore = asyncio.Semaphore(2)
...
async def analyze_repository(
    ...
    async def process_repository():
        ...
        async with repo_analysis_semaphore:
            logger.info(f"Starting repository analysis for {request.url} (user: {current_user.username})")
            
            # Check remaining semaphore count for debugging
            remaining_slots = repo_analysis_semaphore._value
            logger.info(f"Repository analysis semaphore: {remaining_slots} slots remaining")
            
            cloner = GitHubCloner()
            parser = ASTParser()
            chunker = CodeChunker()
            # Using production summarizer with intelligent selection
            ...
            try:
                ...
                yield f"data: {json.dumps({'status': 'chunking_complete', 'message': f'Created {len(all_chunks)} code chunks'})}\n\n"
                
                # Step 4: Generate summaries
                logger.info("About to start summarization")
                yield f"data: {json.dumps({'status': 'summarizing', 'message': 'Generating chunk summaries'})}\n\n"
                
                try:
                    summarized_chunks = await production_summarizer.summarize_chunks_intelligently(all_chunks)
                except Exception as e:
                    logger.error(f"Error during summarization: {e}")
                    # Continue with original chunks if summarization fails
                    summarized_chunks = all_chunks
                    for chunk in summarized_chunks:
                        if not chunk.get('summary'):
                            chunk['summary'] = f"Code component in {chunk.get('language', 'unknown')}"
...
async def get_repository_stats(owner: str, name: str, current_user: User = Depends(get_current_user)):
    """Get statistics for an analyzed repository with user isolation"""
    try:
        graph_service = GraphService()
        repo_name = f"{owner}/{name}"
        
        # Check if user has access to this repository
        user_repos = cache_service.get_user_repositories(current_user)
        has_access = any(repo['name'] == repo_name for repo in user_repos)
